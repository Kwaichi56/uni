...
public sealed class DatabaseAdminViewModel : BaseViewModel
{
    ...
    public DatabaseAdminViewModel()
    {
        ...
        SearchCommand = new RelayCommand(async _ => await SearchAsync(),
            _ => !IsBusy && SelectedTable.Name == "Products");
    }
    ...
    public DatabaseTableInfo SelectedTable { get => _selectedTable; private set { _selectedTable = value; OnPropertyChanged(); OnPropertyChanged(nameof(CanEdit)); CommandManager.InvalidateRequerySuggested(); } }
    ...
    public string SearchField { get => _searchField; set { _searchField = value; OnPropertyChanged(); } }
    ...
    private void AddRow()
    {
        ...
        switch (SelectedTable.Name)
        {
            ...
            case "Users":
                ...
                break;
            case "Products":
                row["CategoryId"] = CategoryOptions.FirstOrDefault()?.Id ?? 1;
    ...
    {
        if (SelectedTable.Name == "Products")
        {
            _ = SearchAsync(descending);
            return;
        }
        if (CurrentView?.Table is not DataTable table) return;
    ...
    private async Task SearchAsync(bool descending = false)
    {
        await RunAsync(async () =>
        {
            var table = await DatabaseService.SearchProductsAsync(SearchTerm, SearchField, descending);
            _isResultView = true;
            OnPropertyChanged(nameof(CanEdit));
            CurrentView = table.DefaultView;
            SelectedRow = CurrentView.Count > 0 ? CurrentView[0] : null;
            Status = $"LINQ to Entities: найдено {CurrentView.Count} товаров; сортировка по цене {(descending ? "↓" : "↑")}.";
        });
    }
    ...
    private async Task ShowStoredCommandAsync()
    {
        await RunAsync(async () =>
        {
            ...
            CurrentView = table.DefaultView;
            SelectedRow = CurrentView.Count > 0 ? CurrentView[0] : null;
            Status = "Статистика по категориям получена запросом LINQ to Entities.";
        });
    }
    ...
    private async Task LoadCategoryOptionsAsync()
    {
        ...
        foreach (DataRow row in categories.Rows)
            CategoryOptions.Add(new CategoryOption(Convert.ToInt32(row["Id"]), row["Name"].ToString() ?? string.Empty));
    }
