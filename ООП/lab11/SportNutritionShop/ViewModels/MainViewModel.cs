...
namespace SportNutritionShop.ViewModels
{
    public class MainViewModel : BaseViewModel
    {
        ...
        public ObservableCollection<SavedCart> SavedCarts { get; set; } = new();
        private SavedCart? _selectedSavedCart;
        public SavedCart? SelectedSavedCart { get => _selectedSavedCart; set { _selectedSavedCart = value; OnPropertyChanged(); if (value != null) LoadSavedCart(value); } }
        ...
        private string _minPrice = "0";
        private string _maxPrice = "1000";
        ...
        public RelayCommand SaveCartCommand { get; }
        public ICommand ShowProfileCommand { get; }
        public ICommand ShowDatabaseCommand { get; }
        public ICommand LogoutCommand { get; }
        public event EventHandler? ProfileRequested;
        public event EventHandler? DatabaseRequested;
        ...
        public MainViewModel(UserRole role, string userName)
        {
            ...
            SaveCartCommand = new RelayCommand(_ => SaveCart(), _ => CartItems.Any());
            CartItems.CollectionChanged += (_, _) => SaveCartCommand.RaiseCanExecuteChanged();
            ShowProfileCommand = new RelayCommand(_ => ProfileRequested?.Invoke(this, EventArgs.Empty));
            ShowDatabaseCommand = new RelayCommand(_ => DatabaseRequested?.Invoke(this, EventArgs.Empty), _ => IsAdmin);
        ...
        public void ReloadProducts()
        {
            ...
            var reloaded = DataService.LoadProducts();
            Products.Clear();
            foreach (var product in reloaded) Products.Add(product);
            SelectedProduct = Products.FirstOrDefault(x => x.Id == selectedId) ?? Products.FirstOrDefault();
            LoadFilterCollections();
        ...
        private void PlaceOrder()
        {
            try
            {
                var orderedItems = CartItems.ToList();
                DataService.CreateOrder(CurrentUserName, orderedItems);
            ...
            {
                MessageBox.Show(exception.Message, T("ErrorCaption"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        // Сохранение и загрузка корзины вызываются из привязок ViewModel.
        public void SaveCart()
        {
            var saved = new SavedCart
            {
                Name = $"Корзина {SavedCarts.Count + 1}",
                Items = CartItems.Select(c => new CartItem { Product = c.Product, Quantity = c.Quantity }).ToList()
            };
            SavedCarts.Add(saved);
        }
        public void LoadSavedCart(SavedCart saved)
        {
            CartItems.Clear();
            foreach (var item in saved.Items) CartItems.Add(new CartItem { Product = item.Product, Quantity = item.Quantity });
            OnPropertyChanged(nameof(CartTotal));
        }
