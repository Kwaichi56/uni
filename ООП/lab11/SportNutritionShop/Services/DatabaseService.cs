...
/// <summary>Business operations and UI mapping over repositories and a unit of work.</summary>
public static class DatabaseService
{
    public static string DatabasePath { get; private set; } = "";
    public static string ConnectionString { get; private set; } = "";
    public static int LowStockLimit { get; private set; } = 5;
    private static Func<string, IShopUnitOfWork>? _unitOfWorkFactory;

    public static void ConfigureUnitOfWork(Func<string, IShopUnitOfWork> factory) =>
        _unitOfWorkFactory = factory ?? throw new ArgumentNullException(nameof(factory));

    private static IShopUnitOfWork Open() =>
        (_unitOfWorkFactory ?? throw new InvalidOperationException("Unit of Work не настроен."))(ConnectionString);
    ...
    {
        await using var db = Open();
        var entities = await db.Products.GetCatalogAsync();
    ...
    // The product repository performs filtering and sorting in the database.
    public static async Task<DataTable> SearchProductsAsync(
        string? term, string field, bool descending)
    {
        await using var db = Open();
        return ToDataTable(await db.Products.SearchAsync(term, field, descending), "Products");
    }
    ...
    public static async Task SaveProductsAsync(IEnumerable<Product> products)
    {
        ...
        {
            var entity = product.Id > 0 ? await db.Products.GetByIdAsync(product.Id) : null;
            if (entity is null)
            {
    ...
    {
        await using var db = Open();
        var product = await db.Products.GetByIdAsync(id);
    ...
    public static async Task CreateOrderAsync(string login, IEnumerable<CartItem> cartItems)
    {
        ...
        foreach (var item in items)
        {
            ...
            var product = await db.Products.GetByIdAsync(item.Product.Id)
    ...
    {
        using var db = Open();
        var userRole = db.Users.FirstOrDefault(u => u.Login == login && u.Password == password)?.Role;
    ...
    {
        using var db = Open();
        var user = db.Users.FirstOrDefault(u => u.Login == profile.Login)
            ?? throw new InvalidOperationException("Пользователь не найден.");
    ...
    public static async Task<DataTable> LoadTableAsync(string tableName)
    {
        ...
        {
            "Categories" => ToDataTable((await db.Categories.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "Users" => ToDataTable((await db.Users.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "Products" => ToDataTable((await db.Products.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "Orders" => ToDataTable((await db.Orders.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "OrderItems" => ToDataTable((await db.OrderItems.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "AuditLog" => ToDataTable((await db.AuditLog.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
            "StoredCommands" => ToDataTable((await db.StoredCommands.GetAllAsync()).OrderByDescending(x => x.Id), tableName),
    ...
    {
        await using var db = Open();
        var results = await db.Products.GetLowStockAsync(limit);
    ...
    public static async Task<DataTable> ExecuteStoredCommandAsync(string name, params SqliteParameter[] parameters)
    {
        ...
        await using var db = Open();
        var results = await db.Categories.GetStatisticsAsync();
    ...
    private static async Task SaveRowsAsync<T>(IShopUnitOfWork db, IRepository<T> set, DataTable table) where T : class, new()
    {
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            if (row.RowState != DataRowState.Added)
            {
                ...
                entity = await set.GetByIdAsync(id) ?? throw new InvalidOperationException($"Запись {id} удалена другим пользователем.");
            }
    ...
    private static async Task CopyProductAsync(IShopUnitOfWork db, Product source, ProductEntity target)
    {
        var categoryName = string.IsNullOrWhiteSpace(source.Category) ? "Без категории" : source.Category.Trim();
        var category = await db.Categories.GetByNameAsync(categoryName);
        if (category is null)
        {
    ...
    private static async Task SeedAsync(IShopUnitOfWork db)
    {
        if (await db.Users.AnyAsync(_ => true)) return;
        await using var transaction = await db.BeginTransactionAsync();
        db.Users.Add(new UserEntity { Login = "admin", Password = "123", Role = "Admin", FirstName = "admin" });
        db.Users.Add(new UserEntity { Login = "client", Password = "123", Role = "Client", FirstName = "client" });
